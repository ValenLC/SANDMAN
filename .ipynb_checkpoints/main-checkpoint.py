...
class Window(QMainWindow):
	...
	def __init__(self):
		...
		self.source_file_name = 'noisy_data_0.05_ecart.xlsx_Data4_double.csv' # Input CSV file
	...
	def validateDirectMode(self):
		if self.started:
			...
			for indexTimestamp in range(nbTimestampsToValidate):
				...
				if timestamp == '2016-01-09T23:00':
					self.validateAllValues()
					return
				if (timestamp[8] != '0' and timestamp != self.previous_feedback_timestamp) or timestamp[6] == '2' or timestamp[6] == '3' or timestamp[6] == '4'  or timestamp[6] == '5':
				# or timestamp[5] == '0'
	...
	def tick(self, step=True):
		...
		if new_files:
			...
			for file in new_files:
				...
				if self.last_timestamp == '2016-01-09T22:00': # Start learning after 10 days
					self.learning = True
				# Giving feedback according to the feedback mode and time of previous feedback
				elif self.feedbackMode != FeedbackMode.MANUAL and (self.last_timestamp == '2016-01-09T23:00' or (self.learning and self.lastFeedback == 'null')):
